...
dl.addEventListener('click', () => {
    window.open('../登录页/login.html', '_self')
    dl.classList.replace('dl', 'tc')
...
// 发送请求
$.ajax({
    url: ' https://cnodejs.org/api/v1/topics?Number',  //请求路径
    method: 'GET',
    success: (res) => {
        console.log(res)
...
let wdlxg = document.getElementsByClassName('wdlxg')[0];
let yhdlxg = document.getElementsByClassName('yhdlxg')[0];
...
if (yhm !== null) {
    // youbian
    zzm1.innerHTML = `${yhm}`
    // 并且原本的右侧盒子不显示 用户登录效果盒子显示
    wdlxg.classList.add('bxs')
    yhdlxg.classList.remove('bxs')
    dl.classList.add('bxs')
    tc.classList.remove('bxs')
}

// 点击退出登录 清除本地储存的用户名 此时登录的类名由dl 变成tc
...
tc.addEventListener('click', () => {
    ...
    // 清除所有
    // localStorage.clear()
    wdlxg.classList.remove('bxs')

    yhdlxg.classList.add('bxs')
    tc.classList.add('bxs')
    dl.classList.remove('bxs')
})
