...
// console.log(id);
let id = location.search.split('=')[1];  //取数组下标为1的元素，即id的值
console.log(id);
    ...
    success: (res) => {
        ...
        if (res.success) {
            ...
            plsz.map((a, b) => {
                ...
                let plzz = document.createElement('div');
                plzz.classList.add('plzz')

                let dz = document.createElement('div');
                dz.classList.add('dz')
                let dzs = document.createElement('div');

                let i = document.createElement('i')
                i.classList.add('iconfont')
                i.classList.add('icon-tubiaozhizuomoban-20')
                ...
                if (a.author.loginname === res.data.author.loginname) {
                    lc.appendChild(plzz);
                    plzz.innerHTML = '作者'
                }
                ...
                // 点赞
                // 获取点赞
                dzs.innerHTML = `${a.ups.length}`
                // 获取点赞数组 排序 取前四的背景色改变 反一个新数组
                if (a.ups.length > 5) {
                    pl1.classList.add('dzbjs')
                }
